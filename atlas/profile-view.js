import {characterLinksMarkup} from './character-links.js';
...
export function profileMarkup(e,city,{owner='',elevation={},children=[],cities=[],area=0,totals={}}={}){
 ...
 ${city?`<details class="profile-section" ${e.relatedCharacters?.length?'open':''}><summary>关联角色 · ${e.relatedCharacters?.length||0}</summary>${characterLinksMarkup(e.relatedCharacters)}</details>`:''}
 ${e.capital?`<p class="profile-capital">${city?(e.capitalRole==='regional'?'地区首府':'国家首都'):'首都 · '+esc(e.capital)}</p>`:''}
 ...
 ${e.nameEtymology?`<p class="profile-prototype">名称由来 · ${esc(e.nameEtymology)}</p>`:''}
