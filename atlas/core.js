(function(global){
 ...
 function validateMetadata(p){if(p.relatedCharacters!=null&&(!Array.isArray(p.relatedCharacters)||p.relatedCharacters.length>100||p.relatedCharacters.some(n=>!n||typeof n.name!=='string'||!n.name.trim()||n.name.length>200||['relation','notes','source'].some(k=>n[k]!=null&&(typeof n[k]!=='string'||n[k].length>({relation:200,notes:2000,source:1000}[k]))))))throw Error('关联角色格式无效');if(p.territoryType!=null&&!['land','maritime'].includes(p.territoryType))throw Error('疆域类型必须是陆地或海洋');if(p.nativeNames!=null&&(!Array.isArray(p.nativeNames)||p.nativeNames.length>20||p.nativeNames.some(n=>!n||typeof n.name!=='string'||typeof n.language!=='string'||n.name.length>250||n.language.length>24)))throw Error('其他本地名称格式无效');if(p.belowSeaLevelConfirmed!=null&&typeof p.belowSeaLevelConfirmed!=='boolean')throw Error('负海拔确认必须为布尔值');if(p.includeInNationalTotals!=null&&typeof p.includeInNationalTotals!=='boolean')throw Error('计入国内总数必须为布尔值');if(p.featured!=null&&typeof p.featured!=='boolean')throw Error('明星标记必须为布尔值');if(p.sizeClass!=null&&!['auto',...CITY_CLASSES.map(c=>c.id)].includes(p.sizeClass))throw Error('城市人口等级无效');for(const k of ['population','gdp','lifeExpectancy','literacy','urbanization','elevation'])if(p[k]!=null&&(!Number.isFinite(p[k])||(k!=='elevation'&&p[k]<0)))throw Error('数值字段无效：'+k);for(const k of ['literacy','urbanization'])if(p[k]>100)throw Error(k+' 必须在 0–100%');for(const k of ['id','name','englishName','localName','localRomanization','localLanguage','realName','notes','alias','fullName','abbreviation'])if(p[k]!=null&&typeof p[k]!=='string')throw Error('文本字段无效：'+k);if(!p.name?.trim())throw Error('名称不能为空');if(p.media){for(const m of Object.values(p.media)){if(!m||typeof m.dataUrl!=='string'||m.dataUrl.length>6000000||!(/^(data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/=\r\n]+|data\/media\/assets\/[a-f0-9]{64}\.(png|jpeg|webp))$/.test(m.dataUrl)))throw Error('图片必须是嵌入的 PNG / JPEG / WebP，且不超过 4 MB');if(m.caption!=null&&typeof m.caption!=='string')throw Error('图片说明必须为文本');}}}
 ...
 function cityElevation(city,customDem,globalDem){
  let z=Number.isFinite(city.elevation)?city.elevation:null,source=city.elevationSource||'用户填写',precise=z!==null;
  const recorded=z!==null;
  if(z===null){z=sampleDem(customDem,...city.coordinates);source=customDem?.name||'导入 DEM';precise=z!==null;}
  if(z===null){z=sampleDem(globalDem,...city.coordinates);source='ETOPO 全球概览 · 0.1°';precise=false;}
  const confirmed=recorded&&city.belowSeaLevelConfirmed===true;
  if(z!==null&&z<0&&!confirmed)return {z:0,rawZ:z,source:source+' · 负值未核实，按设定最低 0 m',precise:false,estimatedFloor:true};
  return {z,source,precise};
 }
